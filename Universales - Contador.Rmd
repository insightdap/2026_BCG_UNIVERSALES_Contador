---
title: "Grupo Universales - Contador"
author: "[2026]"
output: 
...
#Importación   --------------------------------------------------------
# data <- read_sheet("1wnxLyEMMxXDLtVPv5BtKX1dAzS0rJZLFDw4-XUuyaKI")
principal <- read_sheet("1KX7OtyGvOR72StII-XS4gaJIK9Fl3kRP_msw0EtyaPY")

# Seguros Universales
data1 <- read_sheet("1MrQAaUfNKvC-3ihHEyb1q2Z9cwBR2dXyz7C0GzBOv7g") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# Hospital Ángeles
data2 <- read_sheet("1nENmasEndrqqa3aBcASpMpbB3RPp1UxcfC0unWS98mA") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# La Legión
data3 <- read_sheet("1lLxPggEL6o4gI0iNAjVldXph42kGy9-2FxApac1t6O0") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# MedHos
data4 <- read_sheet("1Aqk4TtI2oQL_eiSANAtTw6ft0zHRX_AseXD0IIThX6c") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# TREMSA
data5 <- read_sheet("1WWSJLRC2uBT3L2pTu-UGX4MrDrGtHKVXkPG_kHqo0bM") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# Enlaces
data6 <- read_sheet("10muTaoi8raBu0OnMrGYf10yXhwzj2LXCtTfHbaVQW2w") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

# Grupo Sanas
data7 <- read_sheet("1II3UE6Bc2fPjPcpxaAMVC5h5oEQrNYourOWAXdYDHHs") %>% 
  select(1:4,genero,generacion,nivel_jerarquico,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

#Mantener tablas con datos, eliminando vacías.
tablas <- list(data1,data2,data3,data4,data5,data6,data7) |>
  purrr::keep(~ nrow(.x) > 0)

full_data <- bind_rows(tablas)
...
contador_empresa <- principal %>% 
  # filter(!is.na(pais)) %>% # Evitar vacios
  ...
  mutate(moe = round(margen_error_pct(respuestas_Completado, 
                       N = total_respuestas),digits = 2))
...
contador_area <- principal %>% 
  ...
  mutate(label_area1 = paste0(company," - ",area1)) %>% 
  group_by(label_area1,estado) %>% 
  summarise(respuestas = n()) %>% 
  mutate(prop = round(respuestas/sum(respuestas)*100,0)) %>%
  pivot_wider(names_from = estado, values_from = c(respuestas,prop), values_fill = 0) %>% 
  mutate(total_respuestas = rowSums(across(starts_with("respuestas"))),
...
layout_column_wrap(
  ...
  card_body(
   contador_area %>% 
    hchart("bar", hcaes(x = label_area1, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.z:.0f}")) %>%
    hc_add_series(type = "bar",data = contador_area,hcaes(x = label_area1, y = prop_Completado, z = respuestas_Completado),
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.z:.0f} ({point.y:.0f}%)</span>")) %>%
